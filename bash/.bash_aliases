...
alias vf='vim $(fzf)'
alias todo='vim $HOME/Dropbox/Personal/todo.txt'
alias worklog='mkdir -p $HOME/Dropbox/Work/notes/worklog/$(date +%Y_%m) && vim $HOME/Dropbox/Work/notes/worklog/$(date +%Y_%m)/$(date +%Y_%m_%d).md'
...
tmpfile=$( mktemp -t transferXXX ); if tty -s; then basefile=$(basename "$1" | sed -e 's/[^a-zA-Z0-9._-]/-/g'); curl --progress-bar --upload-file "$1" "https://transfer.sh/$basefile" >> $tmpfile; else curl --progress-bar --upload-file "-" "https://transfer.sh/$1" >> $tmpfile ; fi; rm -f $tmpfile > /dev/null; }
